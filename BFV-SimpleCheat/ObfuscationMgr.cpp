...
fb::ClientPlayer* GetPlayerById( int id )
{
	...
	fb::hashtable<_QWORD>* table = (fb::hashtable<_QWORD>*)(pObfuscationMgr + 16);
...
void* DecryptPointer( DWORD64 EncryptedPtr, DWORD64 PointerKey )
{
	...
	if ( (EncryptedPtr & 0x8000000000000000) == 0 )
...
void* GetObfuscationMgr()
{
	...
	*(void**)&JumpToShellcode[6] = ShellCodeAddr;
	...
	} while ( !ValidPointer(ObfuscationMgr) || IsBadReadPtr( ObfuscationMgr, 8 ) );
